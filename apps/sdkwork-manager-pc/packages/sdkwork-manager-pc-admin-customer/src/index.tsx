...
let customerControllerPromise: Promise<SdkworkIamUserAdminController> | null =
  null;
...
function useCustomerController() {
  const [controller, setController] =
    useState<SdkworkIamUserAdminController | null>(null);
...
function useCustomerDirectory(controller: SdkworkIamUserAdminController) {
  ...
  return {
    ...
    loading,
    page,
    pageInfo: controller.getState().listPageInfo,
    q,
    setPage,
    setQ,
    users,
...
function CustomerOverview({
  ...
  useEffect(() => {
    ...
    void Promise.all([users, memberships])
      .then(([userItems, membershipPage]) =>
        setMetrics([
          [messages.routes.directory[0], userItems.length],
          ...(membershipPage
            ? [
                [messages.memberships, membershipPage.items.length] as [
                  string,
                  number,
                ],
              ]
...
function CustomerDirectory({
  ...
  const totalPages =
    ...
    (state.pageInfo?.hasMore ? state.page + 1 : state.page);
  return (
    <section className="manager-operations-page">
      <div className="manager-operations-toolbar">
        ...
        <span className="manager-operations-note">
          {messages.currentPage}: {state.users.length}
        </span>
      </div>
      ...
      {state.loading ? (
        ...
        <div aria-label={messages.routes.directory[0]} className="manager-operations-table-wrap" role="region" tabIndex={0}>
          <table className="manager-operations-table">
            <caption className="sr-only">{messages.routes.directory[0]}</caption>
            <thead>
              <tr>
                <th>User ID</th>
                <th>Name</th>
            ...
            <tbody>
              {state.users.length ? (
                  ...
                  <tr key={user.userId}>
                    ...
                    <td>
                      <span
                        ...
                        data-status={user.status || "unknown"}
                      >
                        {user.status || "-"}
                      </span>
                    </td>
                    <td className="manager-operations-actions">
                      <Link
                        to={`/admin/customers/${encodeURIComponent(user.userId)}`}
                      >
                        {messages.details}
                      </Link>
                    </td>
                ...
                <tr>
                  <td colSpan={6}>{messages.empty}</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="manager-operations-pagination">
        ...
        <button
          disabled={state.page >= Math.max(1, totalPages) || state.loading}
          onClick={() => state.setPage((value) => value + 1)}
          type="button"
        >
          {messages.next}
...
function CustomerDetail({
  ...
  const decodedUserId = decodeURIComponent(
    pathname.split("/").filter(Boolean).at(-1) ?? "",
  );
  const canReadMemberships = hasManagerPermission("commerce.memberships.read");
  const [user, setUser] = useState<SdkworkIamAdminUser | null>(null);
  const [memberships, setMemberships] = useState<
    Awaited<ReturnType<SdkworkMembershipBackendService["listMembers"]>>["items"]
  >([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);
  useEffect(() => {
    ...
    const membershipRequest = canReadMemberships
      ...
      : Promise.resolve(null);
    void Promise.all([userRequest, membershipRequest])
      .then(([item, membershipPage]) => {
  ...
  return (
    <section className="manager-operations-page">
      <div className="manager-operations-toolbar">
        <Link to="/admin/customers/directory">
          ← {messages.routes.directory[0]}
        </Link>
      </div>
      <div className="manager-detail-grid">
        <article>
          <h2>{messages.identity}</h2>
          <dl>
            <div>
              <dt>User ID</dt>
              <dd>{user.userId}</dd>
      ...
      {canReadMemberships ? (
        <>
          ...
          <div aria-label={messages.memberships} className="manager-operations-table-wrap" role="region" tabIndex={0}>
            <table className="manager-operations-table">
              <caption className="sr-only">{messages.memberships}</caption>
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Plan</th>
                ...
                {memberships.length ? (
                  memberships.map((membership) => (
                    <tr key={membership.id}>
                      <td>
                        <Link
                          to={`/admin/memberships/members/${encodeURIComponent(membership.id)}`}
                        >
                          {membership.id}
                        </Link>
                      </td>
                      <td>{membership.planCode}</td>
                      <td>
                        <span
                          className="manager-status-badge"
                      ...
                      <td>
                        ...
                        <small>{membership.expiresAt}</small>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={4}>{messages.noMemberships}</td>
                  </tr>
                )}
              </tbody>
            </table>
...
function CustomerControllerRoute({
  children,
  messages,
}: {
...
export function createSdkworkManagerCustomerAdminContribution(
  locale: string,
  ...
  return {
    ...
    commercial: {
      entitlementKey: "sdkwork.customer.admin",
    ...
    routes: [
      {
        Component: () => (
          <CustomerControllerRoute messages={messages}>
            {(controller) => (
              <CustomerOverview
        ...
        description: messages.routes.overview[1],
        id: "platform.customers.overview",
        label: messages.routes.overview[0],
        navigationGroups: [
          {
            id: "customer-insights",
            label: messages.navigationGroups.insights,
          },
        ],
      ...
      },
      {
        Component: () => (
          <CustomerControllerRoute messages={messages}>
            {(controller) => (
              <CustomerDirectory controller={controller} messages={messages} />
            )}
          </CustomerControllerRoute>
        ),
        description: messages.routes.directory[1],
        id: "platform.customers.directory",
        label: messages.routes.directory[0],
        navigationGroups: [
          {
            id: "customer-records",
            label: messages.navigationGroups.directory,
          },
        ],
