...
export function createSdkworkManagerCustomerAdminContribution(locale: string): AdminModuleContribution {
  ...
  return {
    ...
    routes: [
      { Component: () => <CustomerControllerRoute messages={messages}>{(controller) => <CustomerOverview controller={controller} membershipService={membershipService} messages={messages} />}</CustomerControllerRoute>, description: messages.routes.overview[1], id: "platform.customers.overview", label: messages.routes.overview[0], navigationGroups: [{ id: "customer-insights", label: messages.navigationGroups.insights }], path: "/admin/customers/overview", requiredPermissions: ["iam.users.read"] },
      { Component: () => <CustomerControllerRoute messages={messages}>{(controller) => <CustomerDirectory controller={controller} messages={messages} />}</CustomerControllerRoute>, description: messages.routes.directory[1], id: "platform.customers.directory", label: messages.routes.directory[0], navigationGroups: [{ id: "customer-records", label: messages.navigationGroups.directory }], path: "/admin/customers/directory", requiredPermissions: ["iam.users.read"] },
